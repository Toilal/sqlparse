...
SQL_REGEX = (
    ...
    (r'[0-9_A-ZÀ-Ü][_$#\w]*',
     lambda m: (is_keyword(m.group()), m.group(), m.end())),
    (r'[;:()\[\],\.]', tokens.Punctuation),
...
DATA_REGEX = (
    (r'(^.+?)^(\\.)$', re.DOTALL | re.MULTILINE,
     [lambda m: (tokens.Literal, m.group(1), m.end(1)),
      lambda m: (tokens.Punctuation, m.group(2), m.end(2))]),
)

SQL_REGEX_FLAGS = re.IGNORECASE | re.UNICODE
SQL_REGEX = [(re.compile(rx, SQL_REGEX_FLAGS).match, tt)
             for rx, tt in SQL_REGEX]

DATA_REGEX = [(re.compile(rx, flags).match, tt)
              for rx, flags, tt in DATA_REGEX]
